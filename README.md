...
끝나면 **아이폰·아이패드 앱을 열기만 하면 됩니다.** 맥이 Tailscale 로 기기를 찾아와
자동으로 연결됩니다 — 코드도 QR 도 없습니다. (맥과 기기가 **같은 Tailscale 계정**이면 됩니다.
브리지가 접속한 기기의 Tailscale 계정을 확인하고 같을 때만 열어 줍니다.)

> 자동으로 안 잡히면 화면의 **QR 코드**를 앱 **⚙️ 설정 → QR 스캔** 으로 찍거나,
> 같은 화면의 **IP 와 토큰**을 설정에 직접 넣어도 됩니다.
